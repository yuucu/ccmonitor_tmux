...
echo "   Current status: $(${SCRIPT_DIR}/ccmonitor.sh status)"
echo "   Fancy format: $(CCMONITOR_DISPLAY_FORMAT=fancy ${SCRIPT_DIR}/ccmonitor.sh status)"
echo
...
echo "   Loading plugin..."
if [ -f "ccmonitor.tmux" ]; then
    echo "   ✓ Plugin file exists"
    if [ -x "ccmonitor.tmux" ]; then
...
echo "   Threshold 0.1%: $(CCMONITOR_CPU_THRESHOLD=0.1 ${SCRIPT_DIR}/ccmonitor.sh status)"
echo "   Threshold 5.0%: $(CCMONITOR_CPU_THRESHOLD=5.0 ${SCRIPT_DIR}/ccmonitor.sh status)"
echo "   Threshold 50.0%: $(CCMONITOR_CPU_THRESHOLD=50.0 ${SCRIPT_DIR}/ccmonitor.sh status)"

# Test 4: Detailed process information
echo
echo "4. Detailed process information:"
${SCRIPT_DIR}/ccmonitor.sh info
...
echo "   2. Load plugin: tmux run-shell '${SCRIPT_DIR}/ccmonitor.tmux'"
